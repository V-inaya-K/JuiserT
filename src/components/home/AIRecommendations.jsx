...
import { useCart } from '../../context/CartContext'; // Adjust if your context is elsewhere
...
const AIRecommendations = () => {
  ...
  const handleAdd = (juice) => {
    addToCart({
      id: juice.id,
      name: juice.name,
      price: juice.price || 100,
      quantity: 1,
      customizations: [],
    });
      ...
      <div className="space-y-4 text-black dark:text-black">
        {aiRecommendations.map(recommendation => (
          <div key={recommendation.id} className="bg-white rounded-xl shadow-sm p-4">
            ...
            <div className="mt-3 space-y-3">
              {recommendation.juices.map(juice => (
                <div key={juice.id} className="flex items-center p-2 bg-gray-50 rounded-lg">
                  <Link href={`/juice/${juice.id}`} className="flex items-center flex-grow">
                    <div className="w-12 h-12 bg-gray-200 rounded-lg relative flex-shrink-0">
                      <div className="absolute inset-0 flex items-center justify-center text-gray-400">
                        <span className="material-symbols-rounded">local_drink</span>
                      </div>
            ...
            <div className="mt-3 text-right">
              <button
                onClick={() => handleBundle(recommendation.juices)}
                className="text-accent text-sm font-medium flex items-center ml-auto"
              >
                <span className="material-symbols-rounded text-sm mr-1">shopping_bag</span>
                Order Bundle
              </button>
